...
func TestMaybeAutoFreeze_DebouncesAndLatches(t *testing.T) {
	...
	h.maybeAutoFreeze(acct, breach) // streak 1 — no fire (debounce N=2)
	if len(fk.calls) != 0 {
		t.Fatalf("fired after 1 breach, want debounce; calls=%d", len(fk.calls))
	}
	h.maybeAutoFreeze(acct, breach) // streak 2 — FIRE
	if len(fk.calls) != 1 {
	...
	h.maybeAutoFreeze(acct, breach) // sentinel — must NOT re-fire
	if len(fk.calls) != 1 {
	...
	h.maybeAutoFreeze(acct, clean)  // reset
	h.maybeAutoFreeze(acct, breach) // 1
	h.maybeAutoFreeze(acct, breach) // 2 → fire
	if len(fk.calls) != 2 {
