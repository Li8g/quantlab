...
| Q2 | `slice_score.reason` 是否最终枚举化 | 第二阶段 |
| Q2b | ~~`fatal_reason` 是否最终枚举化~~ | **原型期已关闭**:`mdd_exceeded` / `nav_non_positive`(`resultpkg.FatalReason` + `CrucibleResult.Validate` 闸门，数值另存 `fatal_mdd_value`) |
