...
| 4 | CPCV/PBO 离线原型，用历史 challenger 实证 — **工具+首用 ✅ 2026-06-12**:通用 CSCV 模块 `research/cpcv_pbo/`(纯 numpy,自检校准)+ 首个用例 = MRA A/B 的 15 个选择事件(`mra_ab.py --stage pbo`,结论 mra_ab README §13:三臂选拔均落噪声区间)。**历史 challenger 审计(本 item 原题)待路线裁决**(引擎级回放 vs Python 镜像,见 cpcv_pbo README) | 中，research/ | 无 |
