...
> 状态:**评估完成;A/B 两阶段(ridge + optuna)全部跑完(2026-06-12)。
> 终局判决 = 预注册判决表 row 4(介于其间):不立项 mra_v1、不弃案,等 mainnet
> 实盘偏差数据后复议,证据偏 H0——详 §6 表 item 4 与 `research/mra_ab/README.md` §12**。
...
### 5.2 Optuna 阶段终局:通俗版解读(2026-06-12;数字权威源 = mra_ab README §12)

**实验问的问题**:把现役策略"一根均线偏离 + 一个动量"的价格特征,换成"六个
频段的调音台推子"(短到 6h、长到 8 天,每频段一个权重交给优化器调),表达力
更强了,能不能换来更多 OOS alpha?

**答案:更聪明的模型,考试成绩反而更差了。** 三个发现,按重要性排:

1. **多出来的容量全用在背题上。** B 臂(调音台)的 IS 成绩是 A 臂(现状镜像)
   的 2-3 倍——它确实学到了更多东西;但放到没见过的 OOS 数据上,5 折里 4 折
   输给 A。教科书级过拟合:容量大的模型把历史噪声当规律背了下来。有个表面
   数字(B−A 中位数差 +1.23%)刚好够到立项线,但那是两臂中位数恰好落在不同
   年份上的假象——预注册判决表的第二道关卡(DSR 条款,0/5)正是防这种假象
   设计的,这次它起作用了。**先写死判决标准再跑实验,这个纪律值回票价。**
2. **信号里有真东西,但只有一部分是真的。** ridge 与 TPE 两种完全不同的方法
   独立找到同两条:<6h 波动反着做、12-24h 趋势顺着做——两个证人口供一致,
   这部分大概率是真规律。但更长频段(24-48h、96-192h)两法给出的方向**相反**
   ——口供打架 = 各自在拟合噪声(§10.2 预设的过拟合警报,触发)。
3. **真瓶颈不是模型不够聪明,是手续费。** 实费率下三臂(含现状镜像 A)OOS
   中位数全输给无脑定投。信号真实但微弱,小时级调仓让摩擦像漏水一样把优势
   漏光;本轮已把交易死区宽度交给优化器自己学(§10.3-2),仍救不回来。
   (Caveat:A 臂是简化镜像,不构成对现役 champion 四窗成绩的否证。)

**终局**:不毙掉、不立项——预注册判决表 row 4,存证据,等 mainnet 实盘
偏差数据后复议。

**后续方向(按"瓶颈在哪打哪"排序):**

1. **别再加表达力。** 本轮最值钱的教训:容量不是瓶颈,加容量只喂过拟合;
   "换更花哨的特征/模型"类想法默认搁置。
2. **复议前置 = 验证机械升级(CPCV/PBO,§6 表 #8)。** anchored 5 折高度
   重叠,不是 5 个独立证人;CPCV/PBO 直接回答"这成绩有多大概率是挑出来的
   运气",框架可复用本脚本。
3. **挖 alpha 的方向是"降频"而不是"增维"。** 未用上的线索:h=72 信号比
   h=24 更顺更稳(§10.2 追加测量);信号弱+摩擦贵的组合下,出路是拉长持有
   期摊薄摩擦——往日线级、更宽死区、更低换手探,而非在 1h 上继续精雕。
4. **等 mainnet 实盘偏差数据(预注册的复议扳机)**:真实成交偏差能校准模拟
   器的摩擦假设(10+5bps)偏松还是偏紧,直接决定第 3 条值不值得做。
...
| 4 | `--stage optuna` 实现 + 跑 5 折,按判决表裁决 | ✅ 2026-06-12 | §10.3 修正全吸收(实现注记 README §11)。**终局判决 = row 4(介于其间):不立项、不弃案,等 mainnet 实盘偏差数据复议;证据偏 H0**——B−A median +1.23% 名义达标但是伪影(逐折 B>A 仅 1/5),DSR 条款 0/5 把它拦下(预注册起效);B 的 IS objective 2-3× A 但 OOS 更差=容量被 IS 过拟合吸收;谱形 w0/w1 复现 ridge 锚但 w2/w5 反号=§10.2 过拟合警报触发;实费率三臂 OOS alpha 中位全负。详 README §12 |
| 5 | (若立项)Go 侧 `mra_v1` 策略 | ⏸ 复议 gated | row 4 判决:复议触发条件 = mainnet 实盘偏差数据可用;复议时先上 CPCV/PBO(#8) |
| 6 | (phase 2)GA landscape 平滑性单测 | ⏸ 复议 gated | toy GA vs TPE 同预算,验证 §3.2 论断;非立项前置 |
| 7 | (v2)MAV/波动率通道 bank 化 | ⬜ 远期 | 同手法;本轮刻意不动以隔离变量 |
| 8 | CPCV/PBO 加固 | ✅ 2026-06-12(MRA 部分) | 通用模块 `research/cpcv_pbo/cscv.py`(Bailey-Prado CSCV,自检:噪声→0.58/真优势→0.00)+ `--stage pbo` 跑完 15 个选择事件:**三臂 median PBO 39-47% ≈ 噪声基线,degradation slope 全负(IS 越好 OOS 越差)**——第三个独立角度印证 row 4/H0,连 A 臂(现状形态)的小时级选拔也接近抛硬币。详 mra_ab README §13。历史 challenger Promote 审计(item 4 下半场)待路线裁决:`research/cpcv_pbo/README.md` |
