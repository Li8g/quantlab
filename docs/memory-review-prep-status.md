...
- DeployChampion state consistency was re-reviewed again on 2026-06-06. The major remains open: the deploy write path still does not prove the requested challenger is the active, unretired champion for the target instance's `(strategy_id,pair)`, and `SetActiveChampion` can still write to a retired instance. No source fix was made.
- Regression test follow-up was reviewed on 2026-06-06. Current tests cover several positive/adjacent paths, but most active high/major risks still lack permanent negative regression tests. The largest gaps are Raw validation fail-closed, Retire CAS, fill dedup concurrency/DB unique backstop, terminal-status replay/downgrade, multi-instance account false-freeze, DeployChampion scope/retired guards, and idempotency read-error paths.
...
6. DeployChampion state consistency — reviewed 2026-06-06
   ...
   - Result: still open. `DeployChampionRequest.Validate()` only checks non-empty `challenger_id`; the handler passes that ID straight into `SetActiveChampion`; the repo updates by `instance_id` only. `uq_champion_active` proves there is at most one active champion for a `(strategy_id,pair)`, but deploy never reads that active row or compares its `challenger_id` with the request. Live Tick resolves strategy from the instance and then loads the requested challenger blob directly.
...
7. Test gap follow-up — reviewed 2026-06-06
   ...
   - Result: mostly still open. Existing coverage includes `CrucibleResult` three-state validation, positive RunEpoch/RunReview/OOS tests, DeployChampion happy path, pure `applyRetire` already-retired rejection, sequential fill replay/cross-channel dedup integration, immediate duplicate-terminal agent ack, and single-instance genesis funding. Missing permanent negative tests remain for invalid Raw fail-closed in engine/review, Raw-level cascade order/OOS rejection, Retire DB CAS, `spot_executions` concurrent dedup/unique-violation handling, terminal status downgrade, replay-after-terminal under expiry/frozen, idempotency read-error submit/event paths, DeployChampion mismatch/retired guards, and multi-instance same-account false-freeze.
...
- Major Stage 4C finding (rechecked 2026-06-06): the same DeployChampion gap is also a DB-conditioned state-write bug. `strategy_instances.active_champion_id` is a plain indexed string, and `InstanceRepo.SetActiveChampion` writes it with `WHERE instance_id = ?` only. The write should be conditioned on the target instance not being `retired` and on the requested `challenger_id` matching the active, unretired `champion_histories` row for that instance's `(strategy_id,pair)`.
- Suggested fix: expose a deploy operation that joins or validates instance + champion_history in one repository transaction or one conditional SQL statement; map `RowsAffected == 0` / not found into a 404 or 422 rather than silently accepting stale or cross-scope deployments; add negative tests for retired instance, retired champion, wrong pair/strategy, and never-promoted challenger.
...
- Regression test follow-up finding (rechecked 2026-06-06): existing high/major regression coverage is mostly partial. `internal/resultpkg/validate_test.go` covers `CrucibleResult` state mutexes but not Raw-level sequence invariants; engine/verification tests cover valid replay paths but not invalid Raw fail-closed; `internal/repository/champion_test.go` covers pure `applyRetire` but not repository CAS; `cmd/saas/agentmsg_dedup_integration_test.go` covers sequential replay/cross-channel dedup but not concurrent insert or DB unique backstop; agent tests cover immediate duplicate-terminal but not replay-after-terminal once expired/frozen; and genesis/auto-freeze tests are single-instance only.
- Suggested test follow-up order: add failing tests in this order before source fixes where feasible: Raw fail-closed, Retire CAS, trade status monotonicity, Agent idempotency read-error/replay-after-terminal, fill dedup DB uniqueness/concurrency, multi-instance account false-freeze, DeployChampion scope/retired guards, then CI import-boundary guard for A-1.
