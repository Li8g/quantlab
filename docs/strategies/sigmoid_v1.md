...
| 5 | 死线兜底注入比例 | 0.5 × `macro_inject_usd` | 半量；与 #4 形成"半量 × 半周期 ≈ 月度速率"；意图 A（保守优于补齐）——数据故障时宁可建仓不足；改任一项需同步另一项 |
...
| 10 | 释放数量约束 | DeadBTC×0.10 ∧ FloatBTC×0.20 | Cap 2（FloatBTC×0.20）主导，保护 Sigmoid 目标权重不受单次冲击；Cap 1 是冗余防线（DCA 跑久后几乎不触发）；与 #8/#9 联动决定 DeadBTC 退出速率（熊市连续触发约 5-10 周释放一倍 FloatBTC） |
...
| v1.7 | 2026-05-18 | 决策 #5 确认（值不变，0.5× 保留）；§11 冻结条件细化为"意图 A 保守优于补齐 + 与 #4 协同约束" |
| v1.8 | 2026-05-18 | 决策 #10 确认（值不变，10%/20% 保留）；§11 冻结条件细化为"Cap 2 主导、Cap 1 冗余 + 与 #8/#9 联动" |
| v1.8-final | 2026-05-18 | 全部 13 条 `[INVENTED v1]` 决策评审完成（5 项实际改动 v1.1-v1.3；6 项确认值不变 + 细化冻结条件 v1.4-v1.8；2 项 #1/#3/#9/#13 默认接受未单独记录）。spec 可作为 Phase 4 实施基线。 |
